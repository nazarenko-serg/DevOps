...
echo "16: Initialize the Kubernetes cluster on the control plane node using kubeadm"
sudo kubeadm init --pod-network-cidr 192.168.0.0/16 --kubernetes-version 1.24.0

echo "17: Set kubectl access"
mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config

echo "18: Install the Calico Network Add-On"
kubectl apply -f https://docs.projectcalico.org/manifests/calico.yaml

echo "19: create the token and copy the kubeadm join command"
